...
| `/books`, `/books/trade`, `/books/sale` | Pública                                                               | `/books` usa `fetchAllBooks` con `scope=all` y catálogo paginado público; las pestañas específicas usan `fetchBooks` con filtros públicos; incluye detalle bibliográfico, dueño, precio, estado e intención | Publicar, buscar un libro, contactar, guardar/favorito y proponer intercambio |
...
- La suite de `BooksPage` comprueba las tres pestañas públicas y la
  normalización de `/books/seeking`, y `Todos` autenticado consulta el catálogo
  agregado sin cargar `/mine`.
