...
const fitBoundsMock = vi.fn((bounds: LeafletBounds) => {
  currentMapBounds = {
    north: bounds[1][0],
    south: bounds[0][0],
    east: bounds[1][1],
...
const getBoundsMock = vi.fn(() => ({
  getNorth: () => currentMapBounds.north,
  getSouth: () => currentMapBounds.south,
  getEast: () => currentMapBounds.east,
  getWest: () => currentMapBounds.west,
}))
const mapEventHandlers = new Map<string, () => void>()
const onMapEventMock = vi.fn((eventNames: string, handler: () => void) => {
  mapEventHandlers.set(eventNames, handler)
})
const offMapEventMock = vi.fn((eventNames: string, handler: () => void) => {
  if (mapEventHandlers.get(eventNames) === handler) {
    mapEventHandlers.delete(eventNames)
  }
})
let currentMapBounds: MapBoundingBox
const mapMock = {
...
beforeEach(() => {
  ...
  onMapEventMock.mockClear()
  offMapEventMock.mockClear()
...
describe('MapCanvas', () => {
  ...
  test('reports pan and zoom bounds without refitting user viewport changes', () => {
    ...
    const reportViewport = mapEventHandlers.get('moveend zoomend')
    expect(reportViewport).toBeDefined()

    // Consume the event generated by the initial programmatic fitBounds.
    reportViewport?.()
    ...
    currentMapBounds = zoomedBbox
    reportViewport?.()

    expect(handleViewportChange).toHaveBeenNthCalledWith(1, pannedBbox)
    expect(handleViewportChange).toHaveBeenNthCalledWith(2, zoomedBbox)
