  ...
  try {
    const result = await client.query(
      ...
      SELECT
        ...
        (SELECT COUNT(*) FROM messages WHERE client_key LIKE 'local-demo-%') AS messages,
        ...
        (SELECT COUNT(*) FROM user_follows user_follow JOIN seeded_users follower ON follower.id = user_follow.follower_id JOIN seeded_users followed ON followed.id = user_follow.followed_id) AS follows,
        (SELECT COUNT(*) FROM community_stories story JOIN seeded_users ON seeded_users.id = story.user_id) AS stories,
        (SELECT COUNT(*) FROM community_comments community_comment JOIN book_listings listing ON listing.id = community_comment.listing_id JOIN seeded_users ON seeded_users.id = listing.user_id) AS comments,
        (SELECT COUNT(*) FROM notifications WHERE idempotency_key LIKE 'local-demo-agreement-%') AS notifications,
        (SELECT COUNT(*) FROM analytics_events WHERE idempotency_key LIKE 'local-demo-analytics-%') AS analytics,
        (SELECT COUNT(*) FROM users WHERE email = ANY($1) AND profile_visibility = 'public' AND location_visibility IN ('city', 'neighborhood')) AS public_profiles,
        ...
        (SELECT COUNT(*) FROM book_listings listing JOIN seeded_users ON seeded_users.id = listing.user_id WHERE NOT EXISTS (SELECT 1 FROM book_listing_images image WHERE image.book_listing_id = listing.id AND image.metadata->>'role' = 'context')) AS listings_without_context_images,
        (SELECT COUNT(*) FROM books book WHERE book.isbn IN (
          '9788437604794','9788437604947','9788499890944','9788478887194','9788491050299',
          '9788437604944','9788420633111','9788408172179','9788416517271','9788497592208',
          '9788401337208','9788499926223','9788498382372','9788445000760','9788417347087',
          '9788497592457','9788497594257','9788497592444','9788417860791','9788439722464',
          '9788483468680','9788439722341','9788418015855','9788497595728','9788439724703',
          '9788413621658','9788413140326','9788466347994','9788423360793','9788423354273'
        ) AND NOT EXISTS (SELECT 1 FROM book_listings listing JOIN seeded_users ON seeded_users.id = listing.user_id WHERE listing.book_id = book.id)) AS books_without_listings,
        (SELECT COUNT(*)
    ...
    if (counts.listings_without_context_images !== 0) {
      throw new Error(
        `Found ${counts.listings_without_context_images} seeded listings without a contextual image.`
      );
    }
    if (counts.books_without_listings !== 0) {
      throw new Error(
        `Found ${counts.books_without_listings} seeded books without a listing.`
      );
    }
